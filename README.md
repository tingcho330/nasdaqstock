...
- **장중 리스크** — `background_risk_manager` (US: `market_hours.SP500.risk_poll_windows`·`sell_time_windows`·`direct_execute`·NYSE 거래일; 장외는 다음 세션까지 대기·스레드 자동 재시작; 잔고 `prpr=0` 시 KIS 실시간 시세 보정·`HHDFS76200200` 폴백; 손절/목표 기준가는 **평단(`pchs_avg_pric`)** 우선)
- **EmergencyDrop·최소보유** — `emergency_drop_pct` **-13%** 급락 시 `min_holding_hours`(336h=14일) **예외** 즉시매도. 그 외 매도는 최소 보유 시간 적용
...
| `risk_manager.py` | 장중 리스크 사이클·`check_sell_condition`·`EmergencyDrop`(min_holding 예외)·평단 기준 손절/목표·옵션 `direct_execute` 즉시매도 |
...
| `trader.py` | 매수/매도·KIS 주문가능금액 동기화·`positions` 손절/목표 우선·회전·`--batch-check-only`; US 매수 지정가+슬리피지·`APBK0952` 재시도 생략·매도 사유 코드(`EMERGENCY_DROP` 등) |
...
| `trading_params.min_holding_hours` | `336` | 매수 후 N시간 미만이면 매도 보류 (**14일**; `EmergencyDrop`은 예외) |
...
| EmergencyDrop·min_holding_hours | ✅ -13% 급락 시 336h 최소보유 예외 · 평단 기준 손절/목표 |
