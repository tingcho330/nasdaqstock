...
> * 상세 문구는 [10. 면책 조항](#10-면책-조항-disclaimer)을 참고하세요.
...
- **US 세션 identity 통일** — `utils.resolve_market_session_identity()`가 `trade_date`(ET 거래일)·`session_open_date_kst`·`session_close_date_kst`를 단일 계산 → capture-open/capture-close/send-summary가 **동일 날짜 축** 사용 (예: 2026-07-16 06:15 KST 실행 → trade_date=20260715, 종료 KST=20260716)
- **일일 요약(US)** — open/close 스냅샷을 **metadata.trade_date 기준으로만 pairing**(파일명 ±1일 연산 금지) → Discord: **일일수익률(USD Primary)**, **보유평가(시장가)**, **예수금 세분화**, **원화 총평가(참고, 선택)**, **환율·원화 분해(선택)**, **미국 거래일·한국 종료일 병기**. `compare_balances`는 **핵심 USD 비교**와 **선택 FX/KRW 분석**을 분리한다 (환율 None으로 전체 비교 실패·잘못된 COMPLETE 승격 금지)
...
│  output/  ← runs/{decision|replay}/…, screener_*_{date}_{am|pm}_SP500.json, gpt_trades_*, trading_data.db │
└─────────────────────────────────────────────────────────────────────────┘
         ▲                              ▲
         │                              │
┌────────┴─────────────┐      ┌─────────┴──────────────────┐
...
| 시각 | 작업 | 실행 |
|------|------|------|
| **22:45** | 장 시작 직전 잔액 스냅샷 | `capture_balance_snapshot("open")` → `canonical/balance_open_trade_{trade_date}.json` |
| **22:50** | 스크리너 | `screener.py --market SP500 --run-mode decision --run-id …` (`integrated_manager`가 명시 전달) |
| **23:30** | 매매 파이프라인 | `health_check` → `news_collector` → `gpt_analyzer` → `trader` (스크리너 DECISION gate 통과 후) |
...
| **06:00** | 장 마감 후 잔액 스냅샷 | `capture_balance_snapshot("close")` → `canonical/balance_close_trade_{trade_date}.json` |
| **06:15** | 일일 요약 | Discord — **동일 trade_date** open/close pair 비교 (US: USD Primary·원화 참고) |
...
`send_daily_trading_summary(target_trade_date=None)`가 `output/daily_balances/`의 open/close 스냅샷을 **같은 `trade_date`끼리만** 짝지어 Discord로 보냅니다.

**세션 identity (US 날짜 3종 통일)** — capture-open/capture-close/send-summary가 각자 날짜를 계산하지 않고 `utils.resolve_market_session_identity()` 하나를 사용합니다.

| 날짜 개념 | 예 (2026-07-16 06:15 KST 실행) | 용도 |
|-----------|-------------------------------|------|
| `trade_date` (US ET 거래일) | `20260715` | **파일 키·pairing 기준** |
| `session_open_date_kst` | `20260715` | 개장(22:30/23:30 KST) 달력일 |
| `session_close_date_kst` | `20260716` | 마감(05:00/06:00 KST) 달력일 — 표시용 |

**daily balance 파일 레이아웃**

| 구분 | 경로 | 성격 |
|------|------|------|
| **canonical (primary)** | `daily_balances/canonical/balance_{open\|close}_trade_{trade_date}.json` | 신규 캡처가 기록·조회 1순위 |
| legacy (호환 기록) | `daily_balances/balance_{open\|close}_{trade_date}.json` | 캡처 시 함께 기록 (기존 리더 호환) |
| legacy (읽기 전용 fallback) | `daily_balances/balance_{open\|close}_{YYYYMMDD}.json` | 파일명 날짜가 trade_date일 수도 KST alias일 수도 있음 — **metadata.trade_date 기준으로만 선택** |

- canonical이 없으면 legacy를 스캔하되 **valid=true·type 일치·`metadata.trade_date` 일치** 파일만 후보이며, 동률 시 canonical=true → legacy_alias=false → `kis_account_snapshot` → 최신 순으로 선택합니다. **파일명에서 ±1일 연산으로 open을 고르지 않습니다.**
- `load_daily_balance_pair(trade_date)`가 open/close의 type·valid·trade_date 동일성을 검증하고, 불일치 시 `DAILY_BALANCE_PAIR_TRADE_DATE_MISMATCH` ERROR로 손익 계산을 중단합니다.
- close 상태는 `CLOSE_SNAPSHOT_PRESENT / MISSING / LEGACY_FALLBACK_USED / CLOSE_CAPTURE_SCHEDULE_MISSED`로 구분합니다. 06:00 canonical close가 있으면 summary는 **capture를 재시도하지 않으며** 스케줄 누락 경고도 내지 않습니다. `SCHEDULE_MISSED`는 "close 없음 + recovery 실패 + 해당 KST 날짜 캡처 evidence 없음"일 때만 발생합니다.
- **과거 스냅샷 재캡처 금지** — 과거 open이 없으면 `OPEN_SNAPSHOT_UNAVAILABLE`로 종료하고 현재 계좌 상태로 재생성하지 않습니다 (`historical_recapture_forbidden`).
- `capture_balance_snapshot()`은 `{"status": "existing_valid"|"created"|"failed", "trade_date", "path", "source"}`를 반환하며, summary recovery는 **반환된 path를 재로드**합니다.
- 요약 성공 시 `daily_balances/daily_summary_meta_{trade_date}.json`에 날짜 관계·선택 스냅샷을 기록합니다.

**통화·단위 검증 (US)** — 스냅샷에 `base_currency`, `total_asset_usd`/`available_cash_usd`/`holdings_value_usd`, `*_krw`, `fx_rate_used`, `value_semantics`를 명시 저장하고, 비교는 **USD 필드끼리만** 수행합니다. legacy 스냅샷은 `total_balance ↔ cash+holdings` 정합성 검사를 통과해야 하며, 단위 불명확·통화 혼합 의심 시 `DAILY_BALANCE_CURRENCY_AMBIGUOUS`로 증감·수익률 산술을 생략한 **PARTIAL** 요약을 보냅니다 (장애 ERROR가 아님).

**`compare_balances` 구조 (US)** — 핵심 USD와 선택 FX/KRW를 **분리**합니다. 환율 None을 `0`으로 치환하지 않으며, FX 실패로 핵심 USD 결과를 유실하지 않습니다.

| 단계 | 입력 | 산출 | 실패 시 |
|------|------|------|---------|
| **A. 핵심 USD** | `total_asset_usd` · `available_cash_usd` · `holdings_value_usd` | `total_change` · `cash_change` · `holdings_change` · `daily_return_pct` · open/close 잔액 | `analysis_success=false`, 구조화 실패 결과 (빈 `{}` 반환 **금지**) |
| **B. 선택 FX/KRW** | `kis_summary.bass_exrt` · `tot_evlu_amt_krw` · `krw_cash` | `trading_pnl_krw` · `fx_impact_krw` · 환율 표시 | `fx_metrics_available=false`, `fx_calculation_status=FX_RATE_INCOMPLETE` — **핵심 USD는 유지** |

**요약 상태 (`summary_status`)**

| 상태 | 조건 | CLI exit |
|------|------|----------|
| **COMPLETE** | pair 자격 + `analysis_success` + 핵심 4 metric 숫자 + (US) FX 지표 가용 + `analysis_error` 없음 | `0` |
| **PARTIAL** | 통화 ambiguous, FX 환율 누락, 현금흐름 증거 부족으로 투자수익률만 미확정, invariant demotion 등 | `0` |
| **FAILED** | 스냅샷 부재·pair trade_date 불일치·Discord 전송 예외 등 | `1` |

- `pair_supports_complete_summary`는 **Open/Close 입력 자격만** 판정합니다. 이것만으로 COMPLETE로 승격하지 않습니다.
- `return_metrics_available` 필드 누락 시 기본값은 **`false`** 입니다 (계산 성공이 명시될 때만 `true`).
- **자산 단순 증감 ≠ 투자수익률.** 외부 입출금·환전 증거가 없으면 `return_calculation_status=CASH_FLOW_EVIDENCE_INCOMPLETE` — `total_change` 등은 유지하고 `investment_return_pct`·`daily_asset_pnl`만 `null`.
- Open/Close FX 중 하나라도 유효 양수가 아니면 `FX_RATE_INCOMPLETE` WARNING → **PARTIAL** (핵심 USD metric은 `return_metrics_available=true`로 유지).
- 저장·Discord 전송 직전 `enforce_summary_state_invariants`가 모순 조합(`COMPLETE`+`total_change=null` 등)을 차단하고 PARTIAL로 demotion합니다 (WARNING, exit 0).

**US (`MARKET=SP500`)** — Discord 표기. 핵심 USD는 스냅샷 canonical 필드, FX/KRW는 `kis_summary` / `summary_YYYYMMDD.json` (선택).

| Discord 필드 | 소스 | 표기 통화 | 비고 |
|--------------|------|-----------|------|
| **일일 수익률 (USD)** ← Primary | `total_asset_usd` close−open | **USD** | 핵심 — FX와 독립 |
| USD 총자산 (예수금+보유) | `total_asset_usd` | **USD** | 핵심 |
| 보유평가 (USD, 시장가) | `holdings_value_usd` | **USD** | 핵심 |
| 예수금 (USD) | `available_cash_usd` (+ close `kis_summary` 세분화) | **USD** | 핵심 (+ 선택 세분화) |
| 총평가 (원화환산, 참고) | `tot_evlu_amt_krw` | **원화** | 선택 — 값 있을 때만 |
| 환율 · 원화 분해 | `bass_exrt`, `trading_pnl_krw`, `fx_impact_krw` | **원화** | 선택 — 양쪽 환율 유효 시에만 |
| 실현 손익 (KIS) | `ovrs_rlzt_pfls_amt` delta | **USD** | 표시 가능 (자산 수익률과 독립) |
...
- **일일 수익률 Primary는 USD**입니다. 원화 총평가·환율 분해는 참고용이며 누락 시 Discord에서 생략합니다. US 요약에서는 혼합 단위 추정 **추정 수수료** 필드를 생략합니다.

**KR** — 스냅샷 `total_balance`·`cash`·`holdings_value`(예수금+보유평가 합산) 및 `holdings_detail` 기반 보조 지표를 사용합니다.

| 공통 | 설명 |
|------|------|
| **open/close 짝** | **`metadata.trade_date`가 같은 open + close만** pairing (US: 파일명이 `balance_open_20260716.json`이라도 trade_date=20260715면 20260715 요약에 사용). KST alias 파일은 더 이상 생성하지 않음 |
| **재전송** | open/close JSON만 있으면 `--send-summary [--trade-date YYYYMMDD]`로 Discord 재전송 가능 |
| **표시 날짜** | US 요약은 **미국 거래일**과 **한국 기준 종료일**을 병기 (예: 2026-07-15 / 2026-07-16) |
...
# 특정 미국 거래일 지정 (capture-close·rebuild에도 동일 옵션)
docker compose exec integrated_manager python /app/run_integrated_manager.py \
  --send-summary --trade-date 20260715
docker compose exec integrated_manager python /app/run_integrated_manager.py \
  --capture-close --trade-date 20260715   # 이미 valid close가 있으면 skip (existing_valid)

# legacy daily balance → canonical 레이아웃 마이그레이션 (복사만, legacy 삭제 없음)
docker compose exec integrated_manager python /app/run_integrated_manager.py \
  --migrate-daily-balance-layout --trade-date 20260715 --dry-run
docker compose exec integrated_manager python /app/run_integrated_manager.py \
  --migrate-daily-balance-layout --trade-date 20260715 --apply
```
...
```
[22:50 screener DECISION]                 [23:30 pipeline, 동일 trade_date·session]
screener.py --run-mode decision           health_check.py (AAPL @ NAS)
  --run-id {YYYYMMDD-HHMMSS-xxxxxx}         → news_collector.py (--file 고정 candidates)
  --date {trade_date} --session pm          → gpt_analyzer.py (--date --session)
  → output/runs/decision/.../{run_id}/      → trader.py (DECISION gate 통과 후)
       manifest.json + inputs/ + *.json        → recorder → trading_data.db
  → (성공 시에만) Production 고정 파일 갱신
       screener_candidates_{date}_pm_SP500.json
       screener_scores_* / run_meta_* / review
  → latest/screener_decision_SP500_pm.json
  → logs/screener_{date}_{session}_SP500_{run_id}.log
         └──────────────────────────────────────────┘
...
| 모드 | 누가 | Production 고정 파일 | trader 입력 |
|------|------|---------------------|-------------|
| **DECISION** | `integrated_manager` 정규 스케줄 (`--run-mode decision` 명시) | 성공 시 atomic 갱신 | ✅ (`decision_artifact=true`) |
| **REPLAY** | 직접 CLI 기본값 · `--force` · 장 마감 후/과거일 재계산 | **절대 미갱신** | ❌ |

- Production 덮어쓰기(기존 고정 파일·과거 trade_date DECISION): `--run-mode decision` **+** `--allow-decision-overwrite` 둘 다 필요
- REPLAY 기본 `replay_type=CURRENT_DATA_RECALCULATION` (현재 조회 가능 데이터로 재계산 — 원본 DECISION과 달라질 수 있음). `FROZEN_INPUT_REPLAY` 실행은 미구현(입력 snapshot만 보존)
- `integrated_manager`는 스크리너 exit 0 후 manifest/`run_mode=DECISION`·candidate count·SHA를 검증하고, 실패 시 trader로 진행하지 않음

**`PIPELINE_SCRIPTS` (의존성 순):**

1. `health_check.py`
2. `news_collector.py` ← 당일 `pm` **DECISION** 스크리너 JSON (`--file` 명시)
3. `gpt_analyzer.py` ← `collected_news_{date}_{session}_SP500.json` (`summary_*.json` 있으면 USD Budget Guard)
4. `trader.py` ← `gpt_trades_{date}_{session}_SP500.json` (고정 candidates의 DECISION meta 재검증)

스케줄상 잔액 스냅샷은 `23:55`(open)·`06:00`(close)에 `account.py`를 호출해 저장하며, 로컬 GPT 테스트 시에는 파이프라인 직전에 `account.py`를 한 번 실행하는 것을 권장합니다.

레거시 파일(`_{date}_SP500.json`, 세션 접미사 없음)도 `find_latest_file()`이 호환합니다. 수동으로 만든 `output/replay/`·`output/decision_runs/` 등은 **legacy evidence**로 두며 새 코드가 삭제·이동하지 않습니다.

### 3.5 주요 산출물 (`output/`)

| 패턴 | 모듈 |
|------|------|
| `runs/decision/{MARKET}/{date}/{session}/{run_id}/` | `screener.py` — **불변** DECISION run (manifest·scores·candidates·inputs·review·log) |
| `runs/replay/{MARKET}/{date}/{session}/{run_id}/` | `screener.py` — REPLAY 전용 (고정 Production과 물리 분리) |
| `latest/screener_decision_{MARKET}_{session}.json` | 최신 성공 DECISION 포인터 (atomic JSON, REPLAY 미갱신) |
...
| `logs/screener_{date}_{session}_SP500_{run_id}.log` | `integrated_manager` — 자식 stdout/stderr 전체 (run dir `screener.log`와 병행) |
...
| `daily_balances/canonical/balance_{open\|close}_trade_{trade_date}.json` | `integrated_manager` — **신규 canonical** daily balance (조회 1순위) |
| `daily_balances/balance_{open\|close}_*.json` | `integrated_manager` — legacy daily balance (metadata.trade_date 기준 fallback)·`trade_date`/`source`/`valid`/통화 필드(`total_asset_usd` 등) 메타 |
| `daily_balances/daily_summary_meta_{trade_date}.json` | `integrated_manager` — 일일 요약 날짜 관계·선택 스냅샷 기록 |
...
| `cache/` (`kis_token.json`, `*.mst`, `*.pkl`, `amount5d_*_v2.json`) | KIS·스크리너 |
...
| 파일 | 역할 |
|------|------|
| `integrated_manager.py` | 스케줄, AM/PM·`trade_date` 컨텍스트, subprocess 파이프라인(**스크리너 stdout 파일 보존·INFO 요약**), open/close 잔액(**KIS snapshot 우선**, canonical 레이아웃·metadata pairing)·`--rebuild-daily-balance`·`--trade-date`·`--migrate-daily-balance-layout`·**US/KR 일일 요약** |
...
| `trader.py` | 매수/매도·**DECISION artifact gate**·live trade_date snapshot·`ACCOUNT_FILE_STALE` vs `ACCOUNT_SYNC_MISMATCH`·`_persist_trade_durable`·sellable clamp·`--batch-check-only` |
...
| 파일 | 역할 |
|------|------|
| `utils.py` | `resolve_market_trade_date`·`resolve_pipeline_context`·`resolve_market_session_identity`, US EXCD/OVRS 맵, buy/sell order params, `risk_session_windows`, `find_latest_file` |
...
| `rsi_overheated_threshold` | `70.0` | RSI 과열 시 `RSI_OVERHEATED` — 신규 후보 제외(스코어 파일에는 유지) |
| `exclude_held_from_candidates` | `true` | 보유종목은 스코어에 남기고 신규 후보에서 제외 |
| `issuer_dedupe_enabled` | `true` | 동일 `issuer_group` 최고점 1종만 유지 |
| `issuer_groups_file` | `issuer_groups.json` | `config/issuer_groups.json` (GOOG/GOOGL→ALPHABET 등) |
...
**US 일일 요약:** `balance_open_time`(기본 `22:45`)·`balance_close_time`(기본 `06:00`) 캡처는 모두 **US trade_date** 키의 canonical 파일로 저장됩니다 (KST alias 파일 미생성). `06:15` 요약은 **같은 trade_date의 open+close pair**를 metadata 기준으로 조회해 Discord에 **USD 일일수익률(Primary)**·**보유평가(시장가)**·**예수금 세분화**·**원화 총평가(참고, 선택)**·**환율 분해(선택)**와 **미국 거래일/한국 종료일**을 표시합니다. USD와 KRW를 섞어 수익률을 계산하지 않습니다. 통화 ambiguous → `DAILY_BALANCE_CURRENCY_AMBIGUOUS` PARTIAL; Close 환율 누락 → 핵심 USD는 유지·`FX_RATE_INCOMPLETE` PARTIAL (TypeError·빈 비교 결과로 COMPLETE 승격하지 않음).
...
```bash
# 스크리너 관련만 필터 (성공 시 INFO에 퍼널·EMPTY·캐시 요약 포함)
docker compose logs -f integrated_manager | grep -iE 'screener|AMOUNT5D|EMPTY|퍼널'

# 전체 stdout은 파일로 보존됨
ls -lt output/logs/screener_*_SP500_*.log | head

# 수동 실행 — 기본 REPLAY (Production 고정 파일 미변경)
...
# → output/runs/replay/SP500/YYYYMMDD/pm/{run_id}/ 만 생성

# Production 고정 파일을 의도적으로 덮어쓸 때만 (위험)
docker compose exec integrated_manager python /app/src/screener.py \
  --market SP500 --date YYYYMMDD --session pm --run-mode decision \
  --allow-decision-overwrite --debug --force

# Amount5D 캐시 무시 재조회 (여전히 기본 REPLAY)
docker compose exec integrated_manager python /app/src/screener.py \
  --market SP500 --date YYYYMMDD --session pm --debug --force --refresh-amount5d

# DECISION run / latest pointer / fixed SHA 확인
...
```bash
# close 스냅샷이 없을 때만 — 이미 valid close가 있으면 existing_valid로 skip
docker compose exec integrated_manager python /app/run_integrated_manager.py \
  --capture-close --trade-date 20260715
# open/close JSON이 있으면 재캡처 없이 metadata.trade_date pair 기준 재계산 후 Discord 전송
docker compose exec integrated_manager python /app/run_integrated_manager.py \
  --send-summary --trade-date 20260715
# legacy → canonical 마이그레이션 (dry-run 먼저, legacy 삭제 없음)
docker compose exec integrated_manager python /app/run_integrated_manager.py \
  --migrate-daily-balance-layout --dry-run
ls -la output/daily_balances/ output/daily_balances/canonical/
```
...
#### A. 스크리너 (선택, 최초 ~3–5분 / 캐시 재실행 단축)

```bash
python3 -u screener.py --market SP500 --date ${DATE} --session ${SESSION} --workers 2 --debug
# 주말·휴장일: --force 추가 (기본 REPLAY — Production 고정 파일 미변경)
# Amount5D 강제 재조회: --refresh-amount5d
# → ../output/runs/replay/SP500/${DATE}/${SESSION}/{run_id}/   (불변 REPLAY)
# 정규 DECISION과 동일 고정 파일을 갱신하려면:
#   --run-mode decision --allow-decision-overwrite
# → ../output/runs/decision/... + screener_candidates_* 등 고정 파일
# → ../output/cache/amount5d_SP500_${DATE}_v2.json
```
...
후보 0건이어도 DECISION/REPLAY run 디렉터리와 (DECISION일 때) 고정 `screener_scores_*`·`screener_run_meta_*`는 남습니다(`result_status=EMPTY_VALID` 등). Shadow·REPLAY 결과는 trader에 전달되지 않습니다.
...
│   ├── issuer_groups.json       # GOOG/GOOGL 등 동일 발행사 매핑
│   ├── .env.example
...
| 스크리너 `--force` (휴장일 테스트) | ✅ 주말·NYSE 휴장일 수동 실행 · **기본 REPLAY**(고정 파일 미덮어쓰기) |
...
| US 세션 identity (`resolve_market_session_identity`) | ✅ trade_date·KST open/close 날짜 단일 계산 (capture·summary 공통) |
| US 일일 요약 open/close 짝 | ✅ **metadata.trade_date pairing** · canonical 레이아웃 · `DAILY_BALANCE_PAIR_TRADE_DATE_MISMATCH` 가드 · `--capture-close --trade-date` |
| daily balance 통화 검증 | ✅ USD 명시 필드(`total_asset_usd` 등)·ambiguous 시 `DAILY_BALANCE_CURRENCY_AMBIGUOUS` PARTIAL (WARNING) |
| `compare_balances` 핵심/선택 분리 | ✅ USD canonical 비교 독립 · FX None-safe · 빈 `{}` 반환 금지 · `FX_RATE_INCOMPLETE` |
| COMPLETE 승격·invariant | ✅ pair 자격≠COMPLETE · `return_metrics_available` 기본 false · `SUMMARY_STATE_INVARIANT_VIOLATION` demotion |
| CASH_FLOW vs 자산 증감 | ✅ `CASH_FLOW_EVIDENCE_INCOMPLETE` 시 투자수익률만 null · `total_change` 유지 |
| 과거 스냅샷 재캡처 금지 | ✅ `historical_recapture_forbidden` · `OPEN_SNAPSHOT_UNAVAILABLE` |
| legacy → canonical migration | ✅ `--migrate-daily-balance-layout --dry-run/--apply` (복사만, 삭제 없음) |
| US 일일 요약 Discord 표기 | ✅ USD Primary·원화/환율 선택 표시·예수금 세분화·`ovrs_rlzt_pfls_amt`·미국 거래일/한국 종료일 병기 |
...
| `Marcap` (US) | 마스터 미제공 → 0 · 필터 **SKIPPED**(`MARKET_CAP_DATA_UNAVAILABLE`) · 대형/중형 PER·PBR 추정에 오용하지 않음 |
