...
- **일일 요약(총평가)** — `balance_open`/`balance_close` 스냅샷을 **예수금+보유평가**로 비교·Discord 전송 (US에서 KIS `tot_evlu_amt_usd`가 예수금만 반영될 때 자동 보정)
...
| **06:15** | 일일 요약 | Discord — open/close **총평가**(예수금+보유평가) 비교 |
...
`send_daily_trading_summary()`가 `output/daily_balances/`의 open/close JSON을 짝지어 Discord로 보냅니다.

| 항목 | 설명 |
|------|------|
| **총평가** | `예수금(dnca_tot_amt)` + `보유평가(evlu_amt 합)`. US에서 KIS `tot_evlu_amt_usd`가 주문가능 예수금과 같으면 보유가 있을 때 위 합산을 우선 |
| **스냅샷 보정** | 과거에 `total_balance`만 예수금으로 저장된 JSON도 요약 시 `holdings_detail`로 재계산 |
| **Discord 필드** | 총평가 변화, 예수금, 보유평가, 일일 수익률·실현/미실현 손익(해당 시) |
...
```bash
# 요약만 재전송 (스냅샷 재캡처 없이, holdings_detail 있으면 총평가 재계산)
...
| `daily_balances/balance_{open\|close}_*.json` | `integrated_manager` 일일 요약 — `total_balance`·`cash`·`holdings_value`·`holdings_detail[]` |
...
| 파일 | 역할 |
|------|------|
| `integrated_manager.py` | 스케줄, AM/PM·`trade_date` 컨텍스트, subprocess 파이프라인, open/close 잔액 스냅샷·**총평가 일일 요약** |
...
**US 일일 요약:** `balance_open_time`(기본 `23:55`) open 스냅샷은 다음 거래일 `session_close_date` 키로도 저장됩니다. `06:15` 요약은 `balance_close_당일` + `balance_open_전일`을 짝지어 **총평가(예수금+보유평가)**·예수금·보유평가 변화를 Discord에 표시합니다. `tot_evlu_amt_usd`만으로는 보유 손익이 0으로 보일 수 있어, 보유 종목이 있으면 합산 총평가를 사용합니다.
...
# open/close JSON이 있으면 재캡처 없이 총평가 재계산 후 Discord 전송
docker compose exec integrated_manager python /app/run_integrated_manager.py --send-summary
...
| US 일일 요약 총평가(예수금+보유평가) | ✅ KIS 예수금-only `tot_evlu` 보정·Discord 분해 표시 |
