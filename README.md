...
- **일일 요약(US)** — open/close 스냅샷을 **metadata.trade_date 기준으로만 pairing**(파일명 ±1일 연산 금지) + `summary_*.json` KIS 필드 → Discord: **일일수익률(USD Primary)**, **보유평가(시장가)**, **예수금 세분화**, **원화 총평가(참고)**, **환율·원화 분해**, **미국 거래일·한국 종료일 병기**
...
**통화·단위 검증 (US)** — 스냅샷에 `base_currency`, `total_asset_usd`/`available_cash_usd`/`holdings_value_usd`, `*_krw`, `fx_rate_used`, `value_semantics`를 명시 저장하고, 비교는 **USD 필드끼리만** 수행합니다. legacy 스냅샷은 `total_balance ↔ cash+holdings` 정합성 검사를 통과해야 하며, 단위 불명확·통화 혼합 의심 시 `DAILY_BALANCE_CURRENCY_MISMATCH`로 증감·수익률·손익 계산을 생략합니다.
...
**US 일일 요약:** `balance_open_time`(기본 `22:45`)·`balance_close_time`(기본 `06:00`) 캡처는 모두 **US trade_date** 키의 canonical 파일로 저장됩니다 (KST alias 파일 미생성). `06:15` 요약은 **같은 trade_date의 open+close pair**를 metadata 기준으로 조회해 Discord에 **USD 일일수익률(Primary)**·**보유평가(시장가)**·**예수금 세분화**·**원화 총평가(참고)**·**환율 분해**와 **미국 거래일/한국 종료일**을 표시합니다. USD와 KRW·단위 불명확 값을 섞어 수익률을 계산하지 않습니다 (`DAILY_BALANCE_CURRENCY_MISMATCH` 시 계산 생략).
...
| daily balance 통화 검증 | ✅ USD 명시 필드(`total_asset_usd` 등)·ambiguous total 시 `DAILY_BALANCE_CURRENCY_MISMATCH` 계산 생략 |
...
| US 일일 요약 Discord 표기 | ✅ USD 수익률 Primary·원화 참고·환율 분해·예수금 세분화·`ovrs_rlzt_pfls_amt`·미국 거래일/한국 종료일 병기 |
...
| `tests/` (performance + broker ledger + daily balance session) | ✅ **106** passed |
