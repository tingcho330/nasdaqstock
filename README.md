...
- **screener_scores 전체 보존** — 최소점수 미달이어도 스코어링 성공 종목 전부 저장(`held`·`exclusion_reasons`·`issuer_group` 포함). trader 입력은 `screener_candidates_*`만
...
screener.py --market SP500              health_check.py (AAPL @ NAS)
  ...
  → screener_scores_{date}_pm_SP500.json  → trader.py (PIPELINE_* env로 gpt_trades 매칭)
     (스코어링 성공 전체)                  → recorder → trading_data.db
  → screener_run_meta_{date}_pm_SP500.json
  → screener_shadow_candidates_* (비교용, trader 미사용)
  → market_state_{date}_pm_SP500.json
  → logs/screener_{date}_{session}_SP500_{run_id}.log  (integrated_manager 보존)
         └──────────────────────────────────────────┘
...
| 패턴 | 모듈 |
|------|------|
| `screener_candidates_{date}_{am\|pm}_SP500.json` | `screener.py` — **trader/news/gpt 입력** (슬림 후보) |
| `screener_candidates_full_*` | `screener.py` — Production 정책 통과 후보 전체 필드 |
| `screener_scores_*` | `screener.py` — **스코어링 성공 전체** (최소점수 미달·보유·과열 포함) |
| `screener_shadow_candidates_*` | `screener.py` — Shadow 동적 임계값 후보 (**trader 미사용**) |
| `screener_run_meta_*` | `screener.py` — 퍼널·점수분포·EMPTY reason·캐시·Shadow 메타 (atomic write) |
| `reviews/screener_review_{date}_{am\|pm}_SP500.md` | `screener.py` — 사람 검토용 Markdown |
...
| `logs/screener_{date}_{session}_SP500_{run_id}.log` | `integrated_manager` — 자식 stdout/stderr 전체 |
...
| 파일 | 역할 |
|------|------|
| `screener.py` | `--market SP500` (기본), `--debug` 퍼널 로그, `--force` 휴장일 테스트, `--refresh-amount5d` 캐시 무시 |
...
**테스트:** `tests/test_performance_review_kis_endpoints.py` + `tests/test_broker_ledger_integrity.py` (broker-only·trade_date·stale account·persist journal 등) + `tests/test_daily_balance_session.py` (세션 identity·open/close pairing·canonical/legacy fallback·통화 검증·migration) + `tests/test_screener_ops.py` (퍼널·Marcap SKIPPED·scores 전체 저장·Shadow·Amount5D 캐시·subprocess 로그) — 전체 **155** 케이스
...
| 파일 | 의미 |
|------|------|
| `screener_scores_*` | 스코어링 성공 **전체** |
| `screener_candidates_full_*` / `screener_candidates_*` | Production 정책 통과 → news/gpt/trader |
| `screener_shadow_candidates_*` | Shadow만 — **trader 입력 금지** |
| `screener_run_meta_*` | 퍼널·분포·EMPTY reason·캐시 hit/miss·Shadow 요약 |
...
> **주말·휴장일 수동 실행:** 기본적으로 스크리너는 **실행 당일(ET) 거래일**만 검사합니다. 토요일·NYSE 휴장일에는 `휴장일이므로 screener를 건너뜁니다`로 종료됩니다. 로컬·Docker 테스트 시 **`--force`** 로 휴장 검사를 건너뛸 수 있습니다. SP500 실행 시 pykrx(KRX)는 **로드하지 않습니다** (KRX 로그인 경고 없음). 동일 `trade_date` 재실행은 Amount5D 캐시를 쓰므로 1차 필터가 크게 단축됩니다.
...
# → ../output/screener_scores_${DATE}_${SESSION}_SP500.json   (스코어링 성공 전체)
# → ../output/screener_run_meta_${DATE}_${SESSION}_SP500.json
# → ../output/screener_shadow_candidates_${DATE}_${SESSION}_SP500.json
# → ../output/reviews/screener_review_${DATE}_${SESSION}_SP500.md
...
후보 0건이어도 `screener_scores_*`·`screener_run_meta_*`는 남습니다(`result_status=EMPTY_VALID`, `empty_reason=MIN_SCORE_THRESHOLD_NOT_MET` 등). Shadow 후보는 trader에 전달되지 않습니다.
...
│   ├── screener.py / screener_core.py / screener_ops.py
...
| subprocess 스크리너 로그 보존 | ✅ `output/logs/screener_*` + INFO 요약 |
...
| `tests/` (performance + broker ledger + daily balance + screener_ops) | ✅ **155** passed |
